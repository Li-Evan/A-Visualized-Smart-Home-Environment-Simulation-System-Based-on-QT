      ...
      m_slice(0)
{
    ...
    chart->setTitle("模拟家居环境-全天能耗情况分析");
    QFont titleFont;
    titleFont.setPixelSize(30);   //参数字号，数字越小，字就越小
...
//    cal_energy_consumption();
    ...
    *m_series << new CustomSlice("房间空调", 10);
    *m_series << new CustomSlice("客厅空调", 10);
    *m_series << new CustomSlice("房间加湿器",10);
    *m_series << new CustomSlice("客厅加湿器", 10);
    *m_series << new CustomSlice("新风系统", 10);
...
//    *m_series << new CustomSlice("房间空调", air_room_consump);
//    *m_series << new CustomSlice("客厅空调", air_livingroom_consump);
//    *m_series << new CustomSlice("房间加湿器", humi_room_consump);
//    *m_series << new CustomSlice("客厅加湿器", humi_livingroom_consump);
//    *m_series << new CustomSlice("新风系统", new_wind_consump);
...
//    chartSettingsLayout->addRow("Animations", m_animationsCheckBox);
    ...
    QGroupBox *chartSettings = new QGroupBox("图表总体设置");
    ...
    seriesSettingsLayout->addRow("水平偏移量", m_hPosition);
    seriesSettingsLayout->addRow("竖直偏移量", m_vPosition);
    seriesSettingsLayout->addRow("饼图大小", m_sizeFactor);
    seriesSettingsLayout->addRow("开始角度", m_startAngle);
    seriesSettingsLayout->addRow("结束角度", m_endAngle);
    seriesSettingsLayout->addRow("中间圆心大小", m_holeSize);
//    seriesSettingsLayout->addRow(appendSlice);
//    seriesSettingsLayout->addRow(insertSlice);
//    seriesSettingsLayout->addRow(removeSlice);
    QGroupBox *seriesSettings = new QGroupBox("展示设置");
    ...
    // 数据序列设置
    m_sliceName = new QLineEdit("请选择饼图上的一块区域");
    ...
    sliceSettingsLayout->addRow("值", m_sliceValue);
    sliceSettingsLayout->addRow("饼图区域间隔", m_pen);
    sliceSettingsLayout->addRow("颜色", m_brush);
    sliceSettingsLayout->addRow("标签是否可见", m_sliceLabelVisible);
    sliceSettingsLayout->addRow("标签字体样式", m_font);
...
//    sliceSettingsLayout->addRow("Label arm length", m_sliceLabelArmFactor);
    sliceSettingsLayout->addRow("分离效果", m_sliceExploded);
    sliceSettingsLayout->addRow("分离距离", m_sliceExplodedFactor);
    QGroupBox *sliceSettings = new QGroupBox("数据序列个性化设置");
